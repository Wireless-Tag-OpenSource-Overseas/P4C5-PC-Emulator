...
Console *console_init(int width, int height)
{
	...
	if (globals.panel_fb) {
		...
		c->fb = globals.panel_fb1 ? globals.panel_fb1 : globals.panel_fb;
	} else {
		   ...
		   int x, int y, int w, int h)
{
	...
	lcd_draw(0, 0, LCD_WIDTH, LCD_HEIGHT, s->fb);

	if (globals.panel_fb1) {
		s->fb = (s->fb == globals.panel_fb) ? globals.panel_fb1 : globals.panel_fb;
		vga_set_fb_data(s->pc->vga, s->fb);
	}
}
...
static int pc_main(const char *file)
{
	...
	fprintf(stderr, "BIOS boot starting...\r\n");
	if (conf.disks[0]) {
		FILE *test = fopen(conf.disks[0], "rb");
		if (test) {
			fseek(test, 0, SEEK_END);
			fprintf(stderr, "hda=%s size=%ld bytes (%ld sectors)\r\n",
				conf.disks[0], ftell(test), ftell(test) / 512);
			fclose(test);
		} else {
			fprintf(stderr, "hda=%s NOT FOUND!\r\n", conf.disks[0]);
		}
	}
	load_bios_and_reset(pc);
	fprintf(stderr, "BIOS loaded, starting emulation\r\n");
	...
	for (; pc->shutdown_state != 8;) {
		...
		if (++step_count % 100000 == 0) {
			fprintf(stderr, "step %lu00k, shutdown_state=%d\r\n",
				step_count / 100000, pc->shutdown_state);
		}
	}
	fprintf(stderr, "Emulation ended, shutdown_state=%d\r\n", pc->shutdown_state);
	return 0;
}
...
int esp32_send_packet(void *data, int len) { return 0; }
